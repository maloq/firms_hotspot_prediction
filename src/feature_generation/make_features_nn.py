...
def prepare_night_light_features_wrapper(
    ...
    coords_array = target_df[['lat_rounded', 'lon_rounded']].to_numpy()
    years = None
    if annual_source_dir is not None or black_marble_source_dir is not None:
        years = _target_year_column(target_df)
        if years is None:
            raise ValueError(
...
def merge_features(
    ...
    if not all_dataframes_to_concat:
        raise ValueError("No feature dataframes available for merging.")
    ...
    if drop_by_sea_mask:
        if 'landseamask' not in final_features_df.columns:
            ...
            keep_mask = final_features_df['landseamask'] < float(landsea_mask_threshold)
            dropped_indices = final_features_df.loc[~keep_mask].index

            final_features_df = final_features_df.loc[keep_mask].reset_index(drop=True)
    ...
    return final_features_df, dropped_indices
...
def generate_all_features(
    ...
    final_df, dropped_indices = merge_features(
    ...
    if not dropped_indices.empty:
        print(f"Dropping {len(dropped_indices)} samples from ts_matrix to align with feature dataframe.")
        ts_matrix = np.delete(ts_matrix, dropped_indices, axis=0)
    ...
    float_cols = list(dict.fromkeys(final_df.select_dtypes(include=["float64"]).columns))
    int_cols = list(dict.fromkeys(final_df.select_dtypes(include=["int64"]).columns))
...
def make_features_and_save(config_or_path: str | dict, output_file: str, test_mode: bool, use_cached_files: bool = False, cache_dir: str = 'climate_features_cache', use_cached_target: bool = False):
    ...
    final_features_df, ts_matrix = generate_all_features(
        ...
        # TODO: Add climate feature generation params
        # Elevation
    ...
    if not final_features_df.empty:
        ...
        np.savez(output_file.replace('.parquet', '.npz'), climate_features=ts_matrix)
        print(f"Climate features saved to '{output_file.replace('.parquet', '.npz')}'.")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", 
        type=str, 
        default="configs/features_config_30d_nn.yaml", # Assuming your config file is YAML
        help="Path to the YAML configuration file."
    )
    parser.add_argument(
        "--output", 
        type=str, 
        default="data/saved_features/train_features_nn_30d.parquet",
